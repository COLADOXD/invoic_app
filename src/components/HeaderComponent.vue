...
const toggleMode = (change: boolean) => {
    if (change) store.isDark = !store.isDark;
    if (store.isDark) {
...
<template>
    <div class="bg-secondary flex justify-between mb-2 fixed w-full">
        <div class="p-5 bg-blue-500 rounded-r-2xl">
        ...
        <div class="flex mr-5 items-center">
            <div @click="toggleMode(true)">
                <div v-if="store.isDark">
                ...
                <div v-else>
                    <svg width="20" height="20" xmlns="http://www.w3.org/2000/svg">
                        <path
                            ...
                            fill="#7E88C3" fill-rule="nonzero" />
                    </svg>
                </div>
            </div>
            <div class="h-full w-[1px] bg-slate-600 mx-4"></div>
            <img class="rounded-full h-12" src="../assets/image-avatar.jpg" alt="">
        </div>
    </div>
</template>